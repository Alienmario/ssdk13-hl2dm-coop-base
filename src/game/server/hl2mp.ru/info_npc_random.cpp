...
LINK_ENTITY_TO_CLASS( info_spawnnode, CBaseEntity );
...
class npcRandom : public CPointEntity
{
	DECLARE_CLASS( npcRandom, CPointEntity );
	DECLARE_DATADESC();
public:
	void Spawn( void );
...
const char *gpPathSearch[] = {
	"custom/hl2mp.ru/maps",
	"custom/hl2/maps",
	"maps",
};

const char *GetFilePath() {
	int nHeads = ARRAYSIZE( gpPathSearch );
	int i;
	for ( i = 0; i < nHeads; ++i )
	{
		const char *szFullName = UTIL_VarArgs( "%s/%s_rndnpc.txt", gpPathSearch[i], STRING( gpGlobals->mapname) );
...
void CC_mp_rndnpc_addspawn( const CCommand &args )
{
	...
	const char *szFullName = GetFilePath();
	if( szFullName == NULL )
		return;
	...
	gpRnd->manifest->SaveToFile( filesystem, szFullName );
}
...
bool npcRandom::FindRandomSpot( Vector *pResult, QAngle *pAngle, const char *szGroup )
{
	...
	if( spawnnode ) {
		static KeyValues *sub;
		if( sub == NULL )
			sub = spawnnode->GetFirstSubKey();

		Vector pKvVector;
		UTIL_StringToVector( pKvVector.Base(), sub->GetString("origin") );
		*pResult = pKvVector + Vector( 0, 0, 16 );

		QAngle pKvAngle;
		UTIL_StringToVector( pKvAngle.Base(), sub->GetString("angles") );
		*pAngle = pKvAngle;

		if( szGroup ) {
			if( strcmpi( szGroup, sub->GetString("group") ) == 0 ) {
				sub = sub->GetNextKey();
				bNoFan = true;
				return true;
			}
...
bool npcRandom::CheckSpotForRadius( Vector *pResult, const Vector &pSpot, CAI_BaseNPC *pNPC, float radius )
{
	...
	UTIL_TraceLine(pSpot, pSpot - Vector( 0, 0, 256 ), MASK_SOLID, pNPC, COLLISION_GROUP_NONE, &tr);
	if( !(tr.contents & CONTENTS_WATER) && tr.fraction != 1.0 && !tr.allsolid && strcmp("**studio**", tr.surface.name) != 0 ) {
		vStartPos = tr.endpos;
		vStartPos.z += 16.0;
	}
	else
