...
void SaveGameStateGlobals( CSaveRestoreData *pSaveData )
{
	...
	// Write global data
	header.version 			= 9057; //Плохая версия
	header.skillLevel 		= 3;	// This is created from an int even though it's a float
	header.connectionCount 	= pSaveData->levelInfo.connectionCount;
	header.time__USE_VCR_MODE	= gpGlobals->curtime;
...
void SaveClientState( CBasePlayer *pPlayer, int pFlags = NULL ) 
{
	...
	while ((ent = gEntList.NextEnt(ent)) != NULL)
	{
		...
		pKV->SetString("velocity", VectorToString( ent->GetAbsVelocity() ) );
		...
		if( ent->IsNPC() ) {
			...
			if( pNpc ) {
				...
				pKV->SetInt("maxhealth", ent->GetMaxHealth() );
...
void RestoreClientState( CBasePlayer *pPlayer ) {
	...
	if( pClient != NULL ) {
		...
		pPlayer->SetModel( pClient->GetString("model", "models/alyx.mdl") );
	}
	...
	if( !( pFlags & FL_SAVERESTORE_NPC_NORESTORE ) && pEntity != NULL ) {
		for( KeyValues *pKV = pEntity->GetFirstSubKey(); pKV != NULL; pKV = pKV->GetNextKey() )
		{
			...
			if( !( pFlags & FL_SAVERESTORE_NPC_NOTP ) ) {
				Vector origin;
				UTIL_StringToVector( origin.Base(), pKV->GetString("origin") );
				QAngle angles;
				UTIL_StringToVector( angles.Base(), pKV->GetString("angles") );
				Vector velocity;
				UTIL_StringToVector( velocity.Base(), pKV->GetString("velocity") );
				pEnt->Teleport( &origin, &angles, &velocity );
			}
			...
			if( pEnt->IsNPC() ) {
				...
				pEnt->KeyValue( "additionalequipment", pKV->GetString("additionalequipment") );
				pEnt->SetHealth( pKV->GetInt("health", 100 ) );
				pEnt->SetMaxHealth( pKV->GetInt("maxhealth", 100 ) );
...
bool SaveGameState( bool bTransition, CSaveRestoreData **ppReturnSaveData )
{
	...
	//Удаляем все ентити которые в очереди на удаление!
